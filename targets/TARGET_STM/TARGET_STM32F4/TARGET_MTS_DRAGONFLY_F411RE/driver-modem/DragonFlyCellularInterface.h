...
#ifndef _DRAGONFLY_CELLULAR_INTERFACE_
#define _DRAGONFLY_CELLULAR_INTERFACE_
...
/**
 * Network registration status (CREG/CGREG Usage)
 */
typedef enum {
...
typedef struct {
    ...
    nsapi_error_t  connection_error_flag;
    bool ppp_connection_up;
    radio_access_nwk_type rat;
    nwk_registration_status reg_status;
} device_info;

/** DragonFlyCellularInterface class
 *
 *  This interface serves as the controller/driver for the MTS
 *  Dragonfly cellular IoT series.
 */
class DragonFlyCellularInterface : public CellularInterface {

public:
    DragonFlyCellularInterface(bool use_USB, bool debugOn=0);
    ~DragonFlyCellularInterface();
     ...
     *  Application should be aborted if NSAPI_ERROR_AUTH_FAILURE is received which means wrong
     *  pin for SIM card.
     *  If continued attempts are made with wrong pin number, SIM card might get locked or blocked
     *  by the operator depending upon regulations.
     *
     *  @param sim_pin     PIN for the SIM card
     ...
     *  Please note that even if 'MBED_CONF_MTS_DRAGONFLY_APN_LOOKUP' config option is set, 'set_credentials()' api still
...
private:
    ...
    bool initiate_nwk_registration();
...
#endif //_DRAGONFLY_CELLULAR_INTERFACE_
